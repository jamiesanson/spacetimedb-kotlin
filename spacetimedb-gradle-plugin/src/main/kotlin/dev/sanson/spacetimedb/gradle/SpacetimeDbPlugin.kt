 ...
 *
 * Registers a `spacetimedb` extension and tasks that build a SpacetimeDB module, extract its
 * schema, and generate typed Kotlin sources. Generated sources are automatically wired into the
 * Kotlin compilation.
 *
 * The plugin also adds `spacetimedb-core` (and transitively `spacetimedb-bsatn`) as a dependency,
 * so consumers don't need to declare it manually.
 *
 * Usage:
 * ```kotlin
 * plugins {
    ...
    override fun apply(project: Project) {
        ...
        val generateTask =
            project.tasks.register(
                ...
                GenerateSpacetimeTypesTask::class.java,
            ) { task ->
        ...
        project.pluginManager.withPlugin("org.jetbrains.kotlin.jvm") {
            ...
            project.dependencies.add(
                "implementation",
                "dev.sanson.spacetimedb:spacetimedb-core:$sdkVersion",
            )
        }
