 ...
 *
 * Runs `spacetime build -p <modulePath>` followed by `spacetimedb-standalone extract-schema <wasm>`
 * to produce the schema JSON.
 *
 * Requires both the `spacetime` CLI and `spacetimedb-standalone` to be available.
 */
public abstract class BuildModuleTask
@Inject
constructor(private val execOperations: ExecOperations) : DefaultTask() {
    ...
    private fun findWasmOutput(moduleDir: File): File {
        ...
        val candidates =
            sequenceOf(releaseDir, debugDir)
                .filter { it.isDirectory }
                .flatMap { dir ->
                    dir.listFiles()?.asSequence()?.filter { it.extension == "wasm" }
                        ?: emptySequence()
                }
    ...
    private fun resolveStandalone(): String {
        ...
        val versionDirs =
            File("$home/.local/share/spacetime/bin")
                .listFiles()
                ?.filter { it.isDirectory }
