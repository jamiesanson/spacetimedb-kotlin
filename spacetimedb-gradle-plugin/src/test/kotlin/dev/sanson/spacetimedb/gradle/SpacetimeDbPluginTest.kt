...
class SpacetimeDbPluginTest {
    ...
    fun `tasks are registered`() {
        ...
        assertTrue(
            result.output.contains("buildSpacetimeModule"),
            "Expected buildSpacetimeModule task",
        )
        assertTrue(
            result.output.contains("generateSpacetimeTypes"),
            "Expected generateSpacetimeTypes task",
        )
    }
    ...
    fun `generates Kotlin files from schema`() {
        ...
        val generatedFiles =
            generatedDir.walkTopDown().filter { it.extension == "kt" }.map { it.name }.toSet()
        ...
        assertTrue(
            "RemoteTables.kt" in generatedFiles,
            "Expected RemoteTables.kt, got: $generatedFiles",
        )
        assertTrue(
            "RemoteReducers.kt" in generatedFiles,
            "Expected RemoteReducers.kt, got: $generatedFiles",
        )
    ...
    private fun copyFixture(target: File) {
        val stream =
            javaClass.classLoader.getResourceAsStream("module-test-v10.json")
    ...
    private fun createTestProject(): File {
        val projectDir =
            File.createTempFile("stdb-test-", "").apply {
                delete()
                mkdirs()
            }
