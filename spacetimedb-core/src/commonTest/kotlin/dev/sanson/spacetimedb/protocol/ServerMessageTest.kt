...
class ServerMessageTest {
    ...
    fun `BsatnRowList rows with FixedSize hint`() {
        ...
        val list = BsatnRowList(sizeHint = RowSizeHint.FixedSize(2u), rowsData = data)
        val rows = list.rows()
    ...
    fun `BsatnRowList rows with RowOffsets hint`() {
        ...
        val list =
            BsatnRowList(sizeHint = RowSizeHint.RowOffsets(listOf(0uL, 2uL, 3uL)), rowsData = data)
        val rows = list.rows()
        assertEquals(3, rows.size)
    ...
    fun `BsatnRowList with zero-size hint returns empty`() {
        val list =
            BsatnRowList(sizeHint = RowSizeHint.FixedSize(0u), rowsData = byteArrayOf(1, 2, 3))
    ...
    fun `BsatnRowList with RowOffsets round-trips through BSATN`() {
        ...
        assertEquals(listOf(0uL, 3uL, 5uL), decoded.sizeHint.offsets)
        assertContentEquals(list.rowsData, decoded.rowsData)
    }
    ...
    fun `PersistentTable has tag 0 and EventTable has tag 1`() {
        val pt: TableUpdateRows =
            TableUpdateRows.PersistentTable(
                BsatnRowList(RowSizeHint.FixedSize(0u), byteArrayOf()),
                BsatnRowList(RowSizeHint.FixedSize(0u), byteArrayOf()),
            )
        val et: TableUpdateRows =
            TableUpdateRows.EventTable(BsatnRowList(RowSizeHint.FixedSize(0u), byteArrayOf()))
    ...
    fun `TransactionUpdateMsg has tag 4`() {
        val msg: ServerMessage =
            ServerMessage.TransactionUpdateMsg(update = TransactionUpdate(emptyList()))
        val bytes = Bsatn.encodeToByteArray(ServerMessage.serializer(), msg)
    ...
    fun `ProcedureResult round-trips through BSATN`() {
        ...
        assertContentEquals(byteArrayOf(0x42), decoded.status.value)
    }
