 ...
 *
 * Mirrors the Rust SDK's `Event` enum. The type parameter [R] represents the
...
public abstract class Event<out R> private constructor() {
     ...
     */
    @Poko
    public class Reducer<out R>(public val event: ReducerEvent<R>) : Event<R>()
     ...
     */
    @Poko
    public class SubscribeError(public val error: SpacetimeError) : Event<Nothing>()
 ...
 *
 * Mirrors the Rust SDK's `ReducerEvent` struct. The type parameter [R] represents
 * the module-specific reducer enum, defined by codegen.
 */
@Poko
...
public abstract class Status private constructor() {
     ...
     */
    @Poko
    public class Failed(public val message: String) : Status()
     ...
     */
    @Poko
    public class Panic(public val message: String) : Status()
}
