...
public class SpacetimeDbConnectionBuilder() {
    ...
    public fun withUri(uri: String): SpacetimeDbConnectionBuilder = apply { this.uri = uri }
     ...
     * corresponding subscription update. This adds latency (dependent on fsync speed) but
     * guarantees that every update the client sees has been persisted.
     *
     * When `false` (the default), updates are sent as soon as the transaction commits, without
     * waiting for durability. This is the recommended setting for real-time applications where low
     * latency matters more than durability guarantees — clients will reconnect and receive fresh
     * state if the server crashes.
     *
     * Defaults to `false`. The SpacetimeDB server defaults V2 protocol clients to `true` if this
     * parameter is not specified, so the SDK explicitly opts out.
     */
     ...
     *
     * Maps table names to their kotlinx.serialization [KSerializer]. This is typically called by
     * generated code.
     */
    public fun withTableDeserializers(
        deserializers: Map<String, KSerializer<out Any>>
    ): SpacetimeDbConnectionBuilder = apply { this.tableDeserializers = deserializers }

    /**
     * Register primary-key extractor functions for tables.
     *
     * Maps table names to a function that extracts the primary-key value from a row. Used to match
     * delete+insert pairs as "update" events. Typically called by generated code.
     */
    public fun withPkExtractors(
        extractors: Map<String, (Any) -> Any>
    ): SpacetimeDbConnectionBuilder = apply { this.pkExtractors = extractors }
     ...
     *
     * When the connection drops unexpectedly, the SDK will retry up to [maxAttempts] times with
     * exponential backoff starting at [initialDelay] and capped at [maxDelay].
     *
     * User-initiated disconnects via [SpacetimeDbConnection.disconnect] are never retried. On
     * reconnect, the cache is cleared and all previous subscriptions are re-established.
     */
    ...
    ): SpacetimeDbConnectionBuilder = apply { this.onConnect = callback }
    ...
    public fun onDisconnect(
        callback: (error: SpacetimeError?) -> Unit
    ): SpacetimeDbConnectionBuilder = apply { this.onDisconnect = callback }

    /** Callback invoked if the initial connection attempt fails. */
    public fun onConnectError(
        callback: (error: SpacetimeError) -> Unit
    ): SpacetimeDbConnectionBuilder = apply { this.onConnectError = callback }

    /**
     * Creates the connection and starts connecting asynchronously.
     *
     * The WebSocket connection is established in a coroutine launched in the provided [scope]. If
     * the connection fails, [onConnectError] is invoked. Cancel the scope or call
     * [SpacetimeDbConnection.disconnect] to shut down.
     *
    ...
    public fun build(scope: CoroutineScope): SpacetimeDbConnection {
        ...
        val dbName =
            requireNotNull(databaseName) {
                "Database name is required. Call withDatabaseName() before build()."
            }
