...
/**
 * Internal BSATN-serialized credential record. Wire-compatible with the Rust SDK's `Credentials {
 * token: String }`.
 */
@Serializable private class CredentialRecord(val token: String)

/**
 * A file on disk which stores (or can store) a JWT for authenticating with SpacetimeDB.
 *
 * The file does not necessarily exist or contain credentials. If credentials have been stored
 * previously, they can be retrieved with [load]. New credentials can be saved to disk with [save].
 *
 * @param key Filename within the credentials directory. Distinct applications on the same machine
 *   can share tokens by using the same key.
 * @param fileSystem The filesystem to use for I/O (inject [okio.fakefilesystem.FakeFileSystem] for
 *   testing).
 * @param baseDir The base directory containing the credentials directory. Defaults to the user's
 *   home directory.
 */
public class CredentialFile(
    ...
    private val credentialsDir: Path
        get() = baseDir / CREDENTIALS_DIR

    private val filePath: Path
        get() = credentialsDir / key

    /**
     * Store the provided [token] to disk.
     *
     * Future calls to [load] on a [CredentialFile] with the same key can retrieve the token.
     */
    ...
    public companion object {
        /**
         * Create a [CredentialFile] using the system filesystem and default credentials directory
         * (`~/.spacetimedb_client_credentials/`).
         *
         * @throws UnsupportedOperationException on platforms without filesystem access (e.g.,
         *   browser JS).
         */
        public fun create(key: String): CredentialFile {
            ...
            return CredentialFile(key = key, fileSystem = systemFileSystem(), baseDir = homeDir)
        }
    }
}
