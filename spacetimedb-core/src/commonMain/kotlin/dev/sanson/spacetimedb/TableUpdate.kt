 ...
 *
 * Used in [TableUpdate] to carry both the deserialized row and the raw bytes needed as the
 * [TableCache] key.
 */
 ...
 *
 * Constructed by the protocol handler after deserializing wire-format messages. Passed to
 * [TableCache.applyDiff] to update the client cache.
 */
 ...
 *
 * Contains the rows that were actually inserted or deleted from the cache (after ref-count
 * deduplication). Call [withUpdatesByPk] to further refine by matching delete+insert pairs that
 * share a primary key into "update" pairs.
 *
...
public class TableAppliedDiff<Row : Any>(
    ...
    /**
     * Match delete+insert pairs that share the same primary key, reclassifying them as updates.
     *
     * Returns a new [TableAppliedDiff] where matched pairs have been moved from [inserts]/[deletes]
     * into [updateInserts]/[updateDeletes].
     */
