...
private val SPACETIMEDB_CONNECTION_BUILDER =
    ClassName("dev.sanson.spacetimedb", "SpacetimeDbConnectionBuilder")
...
/**
 * Generates module-level wiring that ties generated types, table handles, and reducer wrappers
 * together with the core SDK.
 *
 * Produces:
 ...
 */
public class ModuleGenerator(private val schema: ModuleSchema, private val targetPackage: String) {

    /**
     * Generate the table deserializer map file.
     *
     * Produces a top-level function `tableDeserializerMap()` returning `Map<String, KSerializer<out
     * Any>>` with one entry per public table.
     */
    public fun generateDeserializerMapFile(): FileSpec {
        val mapType =
            ClassName("kotlin.collections", "Map")
                .parameterizedBy(
                    ...
                    K_SERIALIZER.parameterizedBy(
                        WildcardTypeName.producerOf(ClassName("kotlin", "Any"))
                    ),
                )

        val mapEntries = CodeBlock.builder().add("mapOf(\n").indent()
        ...
        val funSpec =
            FunSpec.builder("tableDeserializerMap")
                .addModifiers(KModifier.PUBLIC)
                .returns(mapType)
                .addCode("return %L\n", mapEntries.build())
                .build()

        return FileSpec.builder(targetPackage, "TableDeserializerMap").addFunction(funSpec).build()
    }

    /**
     * Generate the PK extractor map file.
     *
     * Produces a top-level function `tablePkExtractorMap()` returning `Map<String, (Any) -> Any>`
     * with one entry per table that has a primary key.
     */
    public fun generatePkExtractorMapFile(): FileSpec {
        ...
        for ((i, table) in tablesWithPk.withIndex()) {
            ...
            val pkFieldName =
                pkElement.name?.toCamelCase()
                    ?: error(
                        "Primary key column at index $pkIndex has no name for table '${table.sourceName}'"
                    )
        ...
        val funSpec =
            FunSpec.builder("tablePkExtractorMap")
                .addModifiers(KModifier.PUBLIC)
                .returns(mapType)
                .addCode("return %L\n", mapEntries.build())
                .build()

        return FileSpec.builder(targetPackage, "TablePkExtractorMap").addFunction(funSpec).build()
    }

    /**
     * Generate a `withModuleDeserializers()` extension on `SpacetimeDbConnectionBuilder` that calls
     * `withTableDeserializers(tableDeserializerMap())` and
     * `withPkExtractors(tablePkExtractorMap())`.
     */
    public fun generateBuilderExtensionFile(): FileSpec {
        val funSpec =
            FunSpec.builder("withModuleDeserializers")
                .addModifiers(KModifier.PUBLIC)
        ...
        return FileSpec.builder(targetPackage, "BuilderExtensions").addFunction(funSpec).build()
    }
    ...
    public fun generateDbConnectionFile(): FileSpec {
        ...
        val classBuilder =
            TypeSpec.classBuilder("DbConnection")
                .addModifiers(KModifier.PUBLIC)
                ...
                .addProperty(
                    PropertySpec.builder("identity", IDENTITY.copy(nullable = true))
                        .addModifiers(KModifier.PUBLIC, KModifier.OVERRIDE)
                        .getter(
                            FunSpec.getterBuilder().addCode("return connection.identity").build()
                        )
                        .build()
                )
                // connectionId
                    ...
                    PropertySpec.builder("connectionId", CONNECTION_ID.copy(nullable = true))
                        ...
                        .getter(
                            FunSpec.getterBuilder()
                                .addCode("return connection.connectionId")
                                .build()
                        )
                        .build()
                )
                ...
                .addProperty(
                    PropertySpec.builder("isActive", Boolean::class)
                        .addModifiers(KModifier.PUBLIC, KModifier.OVERRIDE)
                        .getter(
                            FunSpec.getterBuilder().addCode("return connection.isActive").build()
                        )
                        .build()
                )
                // subscriptionBuilder()
        ...
        return FileSpec.builder(targetPackage, "DbConnection").addType(classBuilder.build()).build()
    }

    /**
     * Generate the per-module `DbConnectionBuilder` class.
     *
     * Wraps [SpacetimeDbConnectionBuilder], auto-configures table deserializers, and returns a
     * generated [DbConnection] from `build()`.
     */
    public fun generateDbConnectionBuilderFile(): FileSpec {
        ...
        val onDisconnectType =
            LambdaTypeName.get(
                parameters =
                    listOf(
                        ParameterSpec.builder("error", SPACETIME_ERROR.copy(nullable = true))
                            .build()
                    ),
                returnType = UNIT,
            )
        val onConnectErrorType =
            LambdaTypeName.get(
                parameters = listOf(ParameterSpec.builder("error", SPACETIME_ERROR).build()),
                returnType = UNIT,
            )

        val classBuilder =
            TypeSpec.classBuilder("DbConnectionBuilder")
                .addModifiers(KModifier.PUBLIC)
                .addProperty(
